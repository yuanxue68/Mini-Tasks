...
const itemListSource = {
  beginDrag(props) {
    return {
      ...
      originalIndex: props.findItemListIndex(props.itemList)
    }
  },
  endDrag(props, monitor){
    const {itemList: droppedItemList, originalIndex} = monitor.getItem()
    if(!monitor.didDrop()){
      props.onMoveItemList(droppedItemList, originalIndex)
    }
  }
}
...
const dropTarget = {
  drop(props, monitor) {
    ...
    if(type==="item"){
      const droppedItem = monitor.getItem().item
      ...
      const droppedItemList = monitor.getItem().itemList
  ...
  hover(props, monitor) {
    ...
    if(type==="item" &&itemList.items.length === 0){
      ...
      onMoveItem(draggedItem, itemList._id, 0)
    } else if(type==='itemList'){
      const {findItemListIndex, onMoveItemList} = props 
      const draggedItemList = monitor.getItem().itemList
    ...
    return connectDropTarget(connectDragSource(
      <div className="item-list">
        <ItemList 
          ...
          draggedListId={dragObj && dragObj.itemList && dragObj.itemList._id}
        />
      </div>
    ))
  }
